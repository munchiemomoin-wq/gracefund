...
async function main() {
  ...
  // Create NEW Categories (inclusive)
  const categories = await Promise.all([
    prisma.category.create({ data: { name: 'Medical & Health', slug: 'medical-health', description: 'Medical treatment, hospital expenses, surgery, disability support, and recovery', icon: 'heart-pulse', sortOrder: 1 } }),
    prisma.category.create({ data: { name: 'Education', slug: 'education', description: 'School fees, college fees, books, uniforms, and educational supplies', icon: 'graduation-cap', sortOrder: 2 } }),
    prisma.category.create({ data: { name: 'Emergency & Family', slug: 'emergency-family', description: 'Family emergencies, food support, rent, housing, and crisis support', icon: 'alert-triangle', sortOrder: 3 } }),
    prisma.category.create({ data: { name: 'Funeral & Memorial', slug: 'funeral-memorial', description: 'Funeral expenses, memorial funds, and family bereavement support', icon: 'flower-2', sortOrder: 4 } }),
    prisma.category.create({ data: { name: 'Children', slug: 'children', description: 'Child welfare, education, essential supplies, and child support programs', icon: 'baby', sortOrder: 5 } }),
    prisma.category.create({ data: { name: 'Community Projects', slug: 'community-projects', description: 'Community development, local initiatives, and public welfare projects', icon: 'users', sortOrder: 6 } }),
    prisma.category.create({ data: { name: 'Charity & Nonprofits', slug: 'charity-nonprofits', description: 'Registered charities, NGOs, and social impact organizations', icon: 'globe', sortOrder: 7 } }),
    prisma.category.create({ data: { name: 'Faith & Religious Organizations', slug: 'faith-religious', description: 'Churches, ministries, religious charities, and faith-based humanitarian projects', icon: 'church', sortOrder: 8 } }),
  ]);

  // Create Users (diverse)
  ...
  await prisma.organization.create({
    data: { ownerId: users[5].id, name: 'Hope Foundation', type: 'ngo', description: 'A registered nonprofit working in education and child welfare across rural India.', country: 'India', city: 'Bangalore', verificationStatus: 'verified', socialLinks: JSON.stringify({ website: 'https://hopefoundation.org' }) },
  });

  // Cover images - diverse imagery (people helping people)
  const covers = [
  ...
  const defaultFeatures = JSON.stringify({ prayer_support: false, donor_messages: true, anonymous_donations: true, campaign_updates: true, beneficiary_verification: false });
  const faithFeatures = JSON.stringify({ prayer_support: true, donor_messages: true, anonymous_donations: true, campaign_updates: true, beneficiary_verification: false });
  ...
  const campaigns = await Promise.all([
    ...
    prisma.campaign.create({
      data: {
        ...
        title: 'Emergency Medical Treatment for a Father',
    ...
    prisma.campaign.create({
      data: {
        ...
        title: 'Provide School Supplies for Children',
        ...
        categoryId: categories[4].id, organizerId: users[3].id, campaignType: 'charity',
    ...
    // 5. Community Food Distribution Program
    prisma.campaign.create({
      data: {
    ...
    prisma.campaign.create({
      data: {
        ...
        story: 'Our community center has been a beacon of hope in Hyderabad for over 15 years. During a recent cyclone, the building suffered catastrophic damage. The roof collapsed, windows were shattered, and the interior was severely damaged.\n\nThe center serves over 500 families, providing free tutoring for underprivileged children, a weekly food distribution program, vocational training for women, and a senior citizens\' activity group.\n\nThe estimated cost of repairs is ₹1,500,000. The community has raised some funds, but we need additional support to complete the restoration.\n\nYour support will help restore not just a building, but a community hub that touches hundreds of lives every week.',
        categoryId: categories[5].id, organizerId: users[5].id, campaignType: 'community',
    ...
    prisma.campaign.create({
      data: {
        ...
        categoryId: categories[3].id, organizerId: users[4].id, campaignType: 'family',
    ...
    // 8. Church Community Outreach Program (faith-based with prayer support)
    prisma.campaign.create({
      data: {
        slug: 'church-community-outreach-program',
        title: 'Church Community Outreach Program',
        shortDescription: 'Grace Community Church runs weekly outreach programs serving meals, tutoring children, and supporting elderly community members.',
        story: 'Grace Community Church has been serving the Hyderabad community for over 30 years. Our weekly outreach program provides free meals to 200+ people, after-school tutoring for 50 children, and companionship visits to elderly residents.\n\nWe are raising funds to expand our outreach to include a job skills training program and a small medical clinic for the community. The total budget for the expansion is ₹1,000,000.\n\nOur programs are open to everyone regardless of background or belief. We believe in serving our community with love and compassion. Every contribution helps us reach more people in need.\n\nYour support directly feeds hungry children, educates the next generation, and brings comfort to those who are lonely.',
        categoryId: categories[7].id, organizerId: users[1].id, campaignType: 'faith_based',
        beneficiaryName: 'Grace Community Church', beneficiaryRelationship: 'Senior Pastor',
        countryId: india.id, stateId: telangana.id, cityId: hyd.id,
        goalAmount: 1000000, raisedAmount: 645000, currency: 'INR',
        status: 'published', verificationLevel: 'organization', isFeatured: false, isUrgent: false,
        campaignFeatures: faithFeatures, coverImage: covers[7],
        donorCount: 189, viewCount: 4560, endDate: new Date('2027-06-30'),
      },
    }),
    // 9. Support a Registered Charity Project
    prisma.campaign.create({
      data: {
        ...
        story: 'Hope Foundation is a registered nonprofit that has been working in child education and welfare for 12 years. Our latest project aims to build a dedicated learning center in a rural district of Karnataka where educational infrastructure is severely lacking.\n\nThe learning center will serve 300+ children annually, providing after-school tutoring, computer literacy classes, a library, and a safe space for learning. The total project cost is ₹2,000,000, of which we have already raised ₹800,000 through grants and individual donors.\n\nWe need an additional ₹1,200,000 to complete construction and equip the center. Our organization is verified, and all financial records are available for review.\n\nEducation is the most powerful tool we can give a child. With your support, we can give hundreds of children the chance to break the cycle of poverty.',
        categoryId: categories[6].id, organizerId: users[5].id, campaignType: 'charity',
  ...
  const messages = ['You are in my thoughts.', 'Sending strength and love.', 'Keep going, you are not alone.', 'Wishing you the very best.', 'Stay strong.', 'Hope this helps.', 'Blessings to you and your family.', 'Thinking of you during this time.', 'Well done for organizing this.', 'Proud to support this cause.'];
  ...
  // Create prayers (only for faith-based campaigns)
  for (const campaign of campaigns.filter(c => c.slug === 'church-community-outreach-program')) {
  ...
  await prisma.platformSettings.createMany({
    data: [
      ...
      { key: 'site_description', value: 'A trusted community crowdfunding platform where people, families, communities, organizations, and meaningful causes come together to raise and give support when it matters most.' },
    ],
  });
