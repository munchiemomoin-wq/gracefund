    ...
    >
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div>
            {campaign.category && (
              <Badge variant="secondary" className="mb-2">{campaign.category.name}</Badge>
            )}
            ...
            <div className="mt-3 flex flex-wrap items-center gap-4 text-sm text-muted-foreground">
              <div className="flex items-center gap-1.5">
                <Avatar className="h-6 w-6">
                  <AvatarFallback className="text-xs bg-primary/10 text-primary">
                    {campaign.organizer?.name?.charAt(0) || 'U'}
                  </AvatarFallback>
                </Avatar>
              ...
              <div className="flex items-center gap-1">
                <Calendar className="h-3.5 w-3.5" /> {new Date(campaign.createdAt).toLocaleDateString('en-IN', { month: 'short', day: 'numeric', year: 'numeric' })}
              </div>
            </div>
          </div>
                  ...
                  <div key={update.id} className="rounded-xl border bg-muted/20 p-4">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-xs text-muted-foreground">
                        {new Date(update.createdAt).toLocaleDateString('en-IN', { month: 'short', day: 'numeric', year: 'numeric' })}
                      </span>
                    </div>
                    {update.content && (
                  ...
                  <div key={d.id} className="flex items-start gap-3 rounded-lg p-2 hover:bg-muted/30 transition-colors">
                    <Avatar className="h-8 w-8 mt-0.5">
                      <AvatarFallback className="text-xs bg-[var(--gold)]/10 text-[var(--gold)]">
                        {(d.isAnonymous || !d.showNamePublicly) ? 'A' : d.donorName.charAt(0)}
                      </AvatarFallback>
                    </Avatar>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-medium">
                          {d.isAnonymous ? 'Anonymous' : (!d.showNamePublicly ? 'A supporter' : d.donorName)}
                        </span>
                      ...
                      {d.donorMessage && (
                        <p className="mt-0.5 text-xs text-muted-foreground italic">"{d.donorMessage}"</p>
                      )}
                    </div>
          ...
          <div className="sticky top-24 space-y-4">
            <div className="rounded-2xl border bg-card p-5 shadow-sm">
              ...
              >
                ...
                {prayed ? 'I Prayed for This' : 'I Prayed for This'}
              </button>
              ...
              <div>
                ...
                <div className="grid grid-cols-4 gap-2">
                  ...
                  <button
                    onClick={handleShare}
                    className="flex h-9 items-center justify-center rounded-lg border text-muted-foreground hover:bg-muted transition-colors"
                    title="Copy Link"
                  >
                ...
                <div className="mt-2 flex items-center gap-3">
                  <Avatar>
                    <AvatarFallback className="bg-primary/10 text-primary">
                      {campaign.organizer?.name?.charAt(0) || 'G'}
                    </AvatarFallback>
                  </Avatar>
                  <div>
